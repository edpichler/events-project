...
class Task1 (events : ArrayList<Event>) {
    val contractModel = ContractModel()

    init {
        events.forEach { contractModel.newEvent(it) }
    }
    ...
    fun getActiveContracts(): Map <Month, List<Contract>> {
        ...
        Month.values().forEach { month ->
            val actives = contractModel.contracts.filter{ it.value.wasActiveAtMonth(month)}.mapValues { it.value }.values
